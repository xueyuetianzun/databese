...
public class JF_view_sysset_subject extends JInternalFrame {
     ...
     * 
     */
    private static final long serialVersionUID = -8647428348671901030L;
    ...
    public void jBsave_actionPerformed(ActionEvent e) {
        int result = JOptionPane.showOptionDialog(null, "是否存盘考试科目信息数据?", "系统提示", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, new String[] {
                "是", "否" }, "否");
        ...
        if (jdbcAdapter.InsertOrUpdateObject(objsubject))
            buildTable();
    ...
    public void jBdel_actionPerformed(ActionEvent e) {
        int result = JOptionPane.showOptionDialog(null, "是否删除考试科目信息数据?", "系统提示", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, new String[] {
                "是", "否" }, "否");
        if (result == JOptionPane.NO_OPTION)
            return;
        String sqlDel = "delete tb_gradeinfo_sub where code = '" + jTextField1.getText().trim() + "'";
        JdbcAdapter jdbcAdapter = new JdbcAdapter();
        jdbcAdapter.DeleteObject(sqlDel);
        sqlDel = "delete tb_subject where code = '" + jTextField1.getText().trim() + "'";
