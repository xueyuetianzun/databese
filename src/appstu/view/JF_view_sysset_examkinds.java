...
public class JF_view_sysset_examkinds extends JInternalFrame {
     ...
     * 
     */
    private static final long serialVersionUID = 5737301996142053263L;
    ...
    public void jBsave_actionPerformed(ActionEvent e) {
        int result = JOptionPane.showOptionDialog(null, "是否存盘考试类别信息数据?", "系统提示", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, new String[] {
                "是", "否" }, "否");
        if (result == JOptionPane.NO_OPTION)
            return;
        
        appstu.model.Obj_examkinds object = new appstu.model.Obj_examkinds();
        object.setKindID(jTextField1.getText().trim());
        object.setKindName(jTextField2.getText().trim());
        JdbcAdapter jdbcAdapter = new JdbcAdapter();
        if (jdbcAdapter.InsertOrUpdateObject(object))
            buildTable();
    ...
    public void jBexit_actionPerformed(ActionEvent e) {
        ...
        int result = JOptionPane.showOptionDialog(null, "是否退出考试类别信息设置?", "系统提示", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, new String[] {
                "是", "否" }, "否");
    ...
    public void jBdel_actionPerformed(ActionEvent e) {
        int result = JOptionPane.showOptionDialog(null, "是否删除考试类别信息数据?", "系统提示", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, new String[] {
                "是", "否" }, "否");
        if (result == JOptionPane.NO_OPTION)
            return;
        String sqlDel = "delete tb_gradeinfo_sub where kindID = '" + jTextField1.getText().trim() + "'";
        JdbcAdapter jdbcAdapter = new JdbcAdapter();
        jdbcAdapter.DeleteObject(sqlDel);
        sqlDel = "delete tb_examkinds where kindID = '" + jTextField1.getText().trim() + "'";
