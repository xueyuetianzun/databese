...
public class Login extends JFrame {
    ...
    public void login() {//进入主菜单事件
    	String password_true = null; String where = " where t_id = "; if (flag.equals("teacher")) {
    	where = " where t_id = ";
    } else {
        where = " where s_id = ";
    }
    try {
        String password_sql = "select passcode from " + flag + where + Integer.parseInt(user.getText()) ;
        Connection conn = demo.Appsu.sendConnection();
...
if (String.valueOf(pswd).equals(password_true)) {//密码
    ...
    teacherMenu.sendID(user.getText());//传递输入的账号ID 用于数据库的操作等
    teacherMenu.init();
    } else {
...
//        ImageIcon icon = new ImageIcon(this.getClass().getResource("/wsy/cl.jpg"));
...
//    }
    public void init() {
        ...
        JPanel panel1 = new JPanel() {//登录界面上半部分
            ...
            ImageIcon image = new ImageIcon("/StudentManage/src/wsy/cl.jpg");//登录界面上部图片
            ...
            JPanel head = new JPanel() {//头像
                private static final long serialVersionUID = 1L;
                ImageIcon icon = new ImageIcon("wsy\\cl.jpg");
                public void paintComponent(Graphics g) {
                    super.paintComponent(g);
            ...
            panel5.add(label2); panel5.add(password);//添加密码输入框
            panel2.add(panel3);
            ...
            button2.addActionListener(e -> System.exit(0));//lambda 表达式
